...
# Phrases that show up on common bot-blocking / interstitial pages. If any of
# these appear in the page text where we expected a login form, that's the
# real cause of failure — not a wrong CSS selector.
BLOCK_INDICATORS = [
    "verify you are human", "unusual traffic", "access denied", "are you a robot",
    "captcha", "checking your browser", "bot detection", "cloudflare",
    "please enable javascript", "blocked",
]
...
def dump_page_diagnostics(page, name):
    """Capture screenshot + URL + title + a text snippet, and flag known bot-block phrases."""
    screenshot(page, name)
    try:
        log.info(f"[{name}] Current URL: {page.url}")
        log.info(f"[{name}] Page title: {page.title()}")
        body_text = page.locator("body").inner_text(timeout=5000).lower()
        hits = [phrase for phrase in BLOCK_INDICATORS if phrase in body_text]
        if hits:
            log.warning(f"[{name}] Possible bot-block/interstitial detected — matched phrases: {hits}")
        # Save first 1000 chars of visible text for manual inspection in the log file.
        log.info(f"[{name}] Page text snippet: {body_text[:1000]!r}")
    except Exception as e:
        log.warning(f"[{name}] Could not extract page diagnostics: {e}")
...
def dismiss_cookie_banner(page):
    """Best-effort click on common cookie/consent banners that could be covering the form."""
    for sel in ["button:has-text('Accept')", "button:has-text('I Agree')", "#cookie-accept", ".cookie-consent button"]:
        loc = page.locator(sel)
        if loc.count() > 0:
            try:
                loc.first.click(timeout=2000)
                log.info(f"Dismissed a banner matching: {sel}")
                time.sleep(1)
            except Exception:
                pass
...
def login(page):
    log.info("Logging in...")
    # networkidle waits for the page's JS/XHR activity to settle, which matters
    # more here than domcontentloaded since Naukri's login form is client-rendered.
    page.goto("https://www.naukri.com/nlogin/login", wait_until="networkidle", timeout=30000)
    time.sleep(2)

    # Always capture what actually loaded, success or not — this is the key
    # diagnostic if the form never appears.
    dump_page_diagnostics(page, "post_goto_login_page")
    ...
    try:
        ...
        dump_page_diagnostics(page, "username_field_not_found")
        raise RuntimeError(
            "Login form never appeared (see username_field_not_found screenshot/log). "
            "Check the page text snippet in the log — if it mentions captcha/verification/blocked, "
            "this is Naukri's bot detection challenging the GitHub Actions runner's IP, not a selector bug."
        )
...
def toggle_summary(page):
    ...
    page.goto("https://www.naukri.com/mnjuser/profile", wait_until="networkidle", timeout=30000)
    time.sleep(3)
    ...
    if edit_btn is None:
        dump_page_diagnostics(page, "summary_edit_button_not_found")
    ...
    if not saved:
        dump_page_diagnostics(page, "save_button_not_found")
...
def run():
    with sync_playwright() as p:
        ...
        try:
            ...
            dump_page_diagnostics(page, "failure")
            print(f"FAILED: {e}")
            raise
        finally:
            context.close()
